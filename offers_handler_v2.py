...
def get_info():
    ...
    keywords, way_of_searching_list, discounts = [], [], []
    ...
    create_final_offer(source_filename, filename_to_create, keywords, discounts,
                       way_of_searching_list)


def create_final_offer(source_filename, filename_to_create, keywords, discounts,
                       way_of_searching_list):
    result = []
    ...
    for name, amount_left, unit, price in zip(df[columns[0]], df[columns[1]], df[columns[2]], df[columns[3]]):
        ...
        for keyword, way_of_searching, discount in zip(keywords, way_of_searching_list, discounts):
            keyword = keyword.upper()
            name = name.replace(",", '.')
            if amount_left != 0:
                new_product = f"{name}, {unit}, {round(price * discount, 2)}\n"
                if way_of_searching == "C":
                    if keyword in name.upper():
                        result.append(new_product)
                        break
                elif way_of_searching == "P":
                    if name.upper().startswith(keyword):
                        result.append(new_product)
                        break
                else:
                    if keyword in [x.upper() for x in name.split(" ")]:
                        result.append(new_product)
                        break
            elif amount_left == 0:
                name += "CHWILOWO NIEDOSTĘPNE"
                new_product = f"{name}, {unit}, {round(price * discount, 2)}\n"
                if way_of_searching == "C":
                    if keyword in name.upper():
                        result.append(new_product)
    ...
    with open(filename_to_create, 'a+', encoding='utf-8') as f:
        f.write(f"Nazwa, Sprzedawane w:, Cena\n")
        for row in result:
            f.write(row)
    ...
    with open(html, 'r', encoding='utf-8') as f:
        ...
        for index in range(len(lines) - 1):
            if "CHWILOWO NIEDOSTĘPNE" in lines[index]:
                lines[index] = lines[index].replace("CHWILOWO NIEDOSTĘPNE", "<br/> CHWILOWO NIEDOSTĘPNE") + "\n"
                lines[index] = lines[index].replace('<td>', '<td style=" animation-duration: 5s; animation-name:alert; animation-iteration-count: infinite;">') + "\n"
    with open(html, 'w', encoding='utf-8') as f:
        f.write("<style> * {font-size: 1.1em;} body {background-color: rgb(155, 155, 101);} table {margin-left: auto;margin-right: auto;border-color: black;border-spacing: 5px;}tr {text-align: center !important;padding: 15px;}th {padding: 15px;border: 3px solid black;margin: 10px 50px;}td {border: 3px solid black;padding: 15px 50px;margin: 10px 50px;}@keyframes alert {from {background-color: rgb(155, 155, 101);}to {background-color: red;}}</style>\n")
        f.writelines(lines)
