...
def change_offer():
    ...
    with open(source_filename, 'r', encoding='utf-8') as f:
        ...
        for line in middle:
            if editing_type == '1':
                while loop:
                    number_to_delete = input("Podaj numer pozycji jaką chcesz usunąć, wpisz 'stop' żeby przestać edytować\n")
...
def get_info():
    while True:
        action = input("Wpisz '1' jeśli chcesz stworzyć nową ofertę\nWpisz '2' jeśli chcesz zmodyfikować istniejącą ofertę\n")
        if action == "1":
            break
        elif action == "2":
            change_offer()
    ...
    while True:
        ...
        for char in illegals_chars:
            if char in filename_to_create:
                print("Podaj poprawną nazwę pliku - nazwa nie może zawierać żadnego z tych znaków: " + ", ".join(illegals_chars))
        else:
            break

    if exists(filename_to_create + ".html"):
        print("Plik o ten nazwie już istnieje - do pliku dodana zostanie data")
        filename_to_create += "-" + datetime.now().strftime("%d/%m/%Y %H:%M").replace("/", "_").replace(" ", "-").replace(":", "_")

    keywords, look_for_match_in_whole_name_list, discounts, low_amounts = [], [], [], []
    ...
    while True:
        ...
        while True:
            ...
            if look_for_match_in_whole_name not in ["c", "C", "P", "p"]:
                ...
                break
        look_for_match_in_whole_name_list.append(look_for_match_in_whole_name)
    ...
    create_final_offer(source_filename, filename_to_create, keywords, discounts, low_amounts, look_for_match_in_whole_name_list)
...
def create_final_offer(source_filename, filename_to_create, keywords, discounts, low_amounts, look_for_match_in_whole_name_list):
    # print("here", keywords, discounts, low_amounts, look_for_match_in_whole_name_list)
    # exit(0)
    result = []
    ...
    for name, amount_left, unit, price in zip(df[columns[0]], df[columns[1]], df[columns[2]], df[columns[3]]):
        ...
        for keyword, low_amount, look_for_match_in_whole_name, discount in zip(keywords, low_amounts, look_for_match_in_whole_name_list, discounts):
